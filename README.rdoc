...
Install the Gajim XMPP Client, http://www.gajim.org, and connect to <i>you@home.com</i> (Be warned that installing Gajim on OS X can painful.
...
is older and does not have a completed Ad-Hoc command implementation). 
...
in the roster and select _execute_ _commands_. _hello_ will be in the list of Ad-Hoc Commands. Select _hello_ from the 
list and click the _forward_ _button_ to execute _hello_.
...
Publish nodes discovered that are not in <tt>agent_xmpp.yml</tt> will be deleted. The following options
...
discovered that are not declared by an event block in <tt>myapp.rb</tt> will be deleted.
...
The routing priority may be configured in <tt>agent_xmpp.yml</tt>. The default value is 1. Valid values are -127
...
Ad-Hoc Commands, jabber:x:data Forms nor Service Discovery are widely supported by XMPP clients and Publish-Subscribe 
is not supported by any client. Gajim http://www.gajim.org provides support for Ad-Hoc Commands and jabber:x:data Forms. Service Discovery,
which is useful for Publish-Subscibe development, is supported by Gajim, but Psi http://psi-im.org provides a much better
implementation. Both Gajim and Psi provide an interface for manual entry of XML messages which is currently required for Publish-Subscribe 
development. Example Publish-Subscribe messages can be found at http://gist.github.com/160344

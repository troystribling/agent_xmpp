...
=== Roster
    ...
    did_receive_update_roster_item_result(pipe, response)

    did_receive_remove_roster_item(pipe, response)

    did_receive_remove_roster_item_error(pipe, response)

    did_receive_update_roster_item_error(pipe, response)
...
=== Applications
    ...
    did_receive_message_normal(pipe, stanza)

    did_receive_pubsub_event(pipe, event, to, from)
...
=== PubSub
    ...
    did_discover_pubsub_service(pipe, jid, ident)
    
    did_discover_pubsub_collection(pipe, jid, node)
      
    did_discover_pubsub_leaf(pipe, jid, node)

    did_receive_pubsub_subscriptions_result(pipe, result)
    
    did_receive_pubsub_subscriptions_error(pipe, result)

    did_receive_pubsub_affiliations_result(pipe, result)
    
    did_receive_pubsub_affiliations_error(pipe, result)

    did_discover_user_pubsub_node(pipe, result)
    
    did_receive_create_node_result(pipe, node, result)    

    did_receive_create_node_error(pipe, node, result)    

    did_receive_pubsub_subscribe_result(pipe, result, node) 

    did_receive_pubsub_subscribe_error(pipe, result, node) 

    did_receive_pubsub_subscribe_error_item_not_found(pipe, result, node) 

    did_receive_pubsub_unsubscribe_result(pipe, result, node) 

    did_receive_pubsub_unsubscribe_error(pipe, result, node) 

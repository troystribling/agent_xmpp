...
Install the Gajim XMPP Client, http://www.gajim.org, and connect to _you@home.com_.

When started for the first time +myapp.rb+ will automatically send buddy requests to all contacts specified
in +agent_xmpp.yml+. Accept the buddy request and _myapp_ will appear as a contact in the Gajim roster. Right click on _myapp_
...
Publish nodes discovered that are not in +agent_xmpp.yml+ will be deleted. The following options
...
discovered that are not declared by an event block in +myapp.rb+ will be deleted.
...
The routing priority may be configured in +agnet_xmpp.yml+. The default value is 1. Valid values are -127
...
implementation. Both Gajim and Psi provide an interface for manual entry of XML messages which is currently required for Publish-Subscribe development.

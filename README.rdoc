...
Agent XMPP allows command authorization groups to be specified by XMPP contact groups. For contacts in the groups <tt>good</tt> and <tt>bad</tt>,
...
By default Agent XMPP executes commands in the main event loop. If a command requires a lot of time for execution it can be deferred to a thread pool. Only the <tt>on(:submit)</tt> calls are deferred.

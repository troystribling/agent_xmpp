  ...
  module SessionMessages
    ...
    def authenticate(pipe)
      if stream_mechanisms.include?('PLAIN')
        Send(Xmpp::SASL.new('PLAIN').auth(pipe.jid, pipe.password))
      else
      ...
      if stream_features.has_key?('bind')
        ...
        Send(iq) do |r|
          if r.type == :result and full_jid = r.first_element('//jid') and full_jid.text
            jid = Xmpp::JID.new(full_jid.text) unless jid.to_s.eql?(full_jid.text)                  
    ...
    def session(stanza)
      if stream_features.has_key?('session')
        ...
        Send(iq) do |r|
          if r.type == :result                
            [Send(Xmpp::Presence.new(nil, nil, 1)), broadcast_to_delegates(:did_start_session, self, stanza)].smash

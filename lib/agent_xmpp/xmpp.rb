require 'agent_xmpp/xmpp/idgenerator'
require 'agent_xmpp/xmpp/xmpp_element'
require 'agent_xmpp/xmpp/xmpp_stanza'
require 'agent_xmpp/xmpp/x'
require 'agent_xmpp/xmpp/jid'
require 'agent_xmpp/xmpp/sasl'

require 'agent_xmpp/xmpp/message'
require 'agent_xmpp/xmpp/presence'
require 'agent_xmpp/xmpp/iq'

require 'agent_xmpp/xmpp/query'
require 'agent_xmpp/xmpp/command'
require 'agent_xmpp/xmpp/version'
require 'agent_xmpp/xmpp/roster'

require 'agent_xmpp/xmpp/data'
require 'agent_xmpp/xmpp/errors'